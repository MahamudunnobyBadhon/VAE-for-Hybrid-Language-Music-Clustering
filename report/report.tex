...
Latent representations $\mu \in \mathbb{R}^{32}$ are extracted for each sample. We apply K-Means ($K\!=\!10$), Agglomerative Clustering (Ward linkage), DBSCAN (eps auto-tuned via silhouette maximization), and HDBSCAN (hierarchical density-based clustering, preceded by UMAP projection to 10 dimensions). PCA + K-Means (baseline) reduces features to 32 dimensions.
...
\subsection{HDBSCAN with UMAP Projection on Hard Task Latents}
\label{sec:hdbscan}

To address the well-known failure of DBSCAN in high-dimensional spaces (see Section~\ref{sec:discussion}), we apply HDBSCAN~\cite{campello2013density} to the pre-trained Hard task latents after first projecting them to 10 dimensions with UMAP ($n\_\text{neighbors}\!=\!30$, $\text{min\_dist}\!=\!0$). Hyperparameters \texttt{min\_cluster\_size} $\in \{50,100,200,300,500\}$ and \texttt{min\_samples} $\in \{5,10,\text{None}\}$ are selected by silhouette score on the projected space; metrics are reported on the original 32-dim latent space using non-noise points only.

\begin{table}[H]
\centering
\caption{HDBSCAN (with UMAP pre-projection) on pre-trained Hard task latents ($N\!=\!20{,}019$). Metrics computed on non-noise samples only. \textbf{Bold} = best per column.}
\label{tab:hdbscan}
\small
\begin{tabular}{lrrrrrrr}
\toprule
\textbf{Model} & \textbf{K} & \textbf{Noise\%} & \textbf{SS $\uparrow$} & \textbf{CHI $\uparrow$} & \textbf{ARI $\uparrow$} & \textbf{NMI $\uparrow$} & \textbf{Purity $\uparrow$} \\
\midrule
BetaVAE+UMAP+HDBSCAN & 4  & 35.3 & \textbf{0.339} & \textbf{10,644} & \textbf{0.255} & \textbf{0.349} & 0.549 \\
CVAE+UMAP+HDBSCAN    & 207 & 2.1  & 0.093          & 553             & 0.027          & 0.250          & \textbf{0.653} \\
MMVAE+UMAP+HDBSCAN   & 17 & 30.2 & 0.155          & 9,794           & 0.213          & 0.344          & 0.607 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{BetaVAE+UMAP+HDBSCAN} yields 4 natural clusters (mcs=300) with ARI = 0.255 and NMI = 0.349---the highest ARI of any single model in this work (surpassing HybridVAE+K-Means ARI = 0.209 and BetaVAE+K-Means ARI = 0.105), despite a 35\% noise fraction. This result confirms that density-based clustering in a well-projected latent space finds genuinely denser genre groupings that K-Means' Euclidean partitioning misses. \textbf{MMVAE+UMAP+HDBSCAN} discovers 17 clusters---closest to the true 18 genre classes---with language purity 87.1\%, matching the K-Means language purity from Section~\ref{sec:multik}. \textbf{CVAE+UMAP+HDBSCAN} over-fragments into 207 micro-clusters (mcs=50): the language+genre conditioning creates highly anisotropic cluster shapes in the projected space, favouring small, tight density peaks rather than coarse genre-level groupings. Importantly, HDBSCAN directly returns the number of natural clusters (no $K$ to specify), providing an unsupervised validation that the Hard task latent spaces contain 4--17 well-separated density modes, consistent with the 10--18 genre classes in the dataset.
...
\paragraph{DBSCAN and HDBSCAN.}
DBSCAN consistently collapsed to 1--2 clusters across all tasks when applied to raw high-dimensional features ($d\!=\!256$): all pairwise $\ell_2$ distances become approximately equal, making density-based neighbourhood estimation unreliable. The misleadingly high Silhouette Score of 0.845 (almost all 20K points in one cluster) is a degenerate outcome. We address this directly in Section~\ref{sec:hdbscan}: applying HDBSCAN to the 32-dim VAE latent space after UMAP projection to 10 dimensions recovers meaningful density structure. BetaVAE+UMAP+HDBSCAN achieves ARI = 0.255---the highest of any single model---confirming that the failure was in applying density-based methods to raw features, not an inherent limitation of density-based clustering on this dataset.
...
\newblock In \textit{Pacific-Asia Conference on Knowledge Discovery and Data Mining (PAKDD)}, pp.\ 160--172, 2013.
