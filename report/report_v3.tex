...
to the pre-trained BetaVAE latent space yields \textbf{ARI\,=\,0.255}---the highest
genre-alignment of any \emph{single unsupervised model}---demonstrating that density-based
clustering recovers meaningful structure when applied in the correct dimensionality regime.
\end{abstract}
...
to the \emph{pre-trained} v1 Hard task checkpoints---no retraining required. Because
HDBSCAN requires low-dimensional input, we first project the 32-dim latent representations
to 10 dimensions with UMAP ($n\_\text{neighbors}\!=\!30$, $\text{min\_dist}\!=\!0$).
...
and NMI\,=\,0.349---the highest ARI of any single model in this work, surpassing
HybridVAE+K-Means (ARI\,=\,0.209) and BetaVAE+K-Means (ARI\,=\,0.105), despite a 35\%
noise fraction. This confirms that density-based clustering in a well-projected latent
space discovers genre groupings that Euclidean K-Means partitioning misses.
...
ARI\,=\,0.255---the highest of any single model---confirming that the failure was in
applying density-based methods to raw features, not an inherent limitation of
density-based clustering on this dataset.
...
\begin{itemize}
    ...
    \item \textbf{HDBSCAN+UMAP on pre-trained BetaVAE latents achieves ARI\,=\,0.255}---best
          single-model genre alignment---without any retraining, confirming that
          density-based clustering succeeds when applied in the low-dimensional
